...
signed main() {
	...
	for (int nxt = 1; nxt <= n; nxt++) {
		for (int sum = nxt; sum <= k; sum++) {
			dp[nxt][sum] = dp[0][sum - nxt];
			for (int pre = 1; pre <= nxt - 2; pre++) dp[nxt][sum] += dp[pre][sum - nxt];
		}
	}
	pre[1] = 1; pre[2] = 2;
	for (int i = 3; i <= n; i++) pre[i] = pre[i - 1] + pre[i - 2] + 1;
